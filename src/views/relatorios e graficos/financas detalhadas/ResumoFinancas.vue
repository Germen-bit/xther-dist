      ...
      createChart(data) {
        ...
        anualChart = new Chart(grafico, {
          ...
          data: {
            ...
            datasets: [
              {
                label: `Finanças Reais`,
                data: data.map((culto) => Number((culto.financas.total).replace(',',''))),
                borderWidth: 2,
                borderJoinStyle: 'round',
                borderColor: '#6657F6'
              },
              {
                label: 'Alvo de Finanças',
                data: data.map((culto) => Number((culto.financas.alvoFinancas).replace(',',''))),
                borderWidth: 2,
